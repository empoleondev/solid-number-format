...
export function memoizeOnce(cb) {
    let lastArgs;
...
export function isNanValue(val) {
    return typeof val === 'number' && isNaN(val);
}
export function isNotValidValue(val) {
...
export function toNumericString(num) {
    ...
    let [coefficient, exponent] = _num.split(/[eE]/g);
 ...
 */
export const findChangeRange = memoizeOnce((prevValue, newValue) => {
    let i = 0, j = 0;
...
export const findChangedRangeFromCaretPositions = (lastCaretPositions, currentCaretPosition) => {
...
export function geInputCaretPosition(el) {
...
export function getDefaultChangeMeta(value) {
    return {
        from: {
            start: 0,
            end: 0,
...
export function getMaskAtIndex(mask = ' ', index) {
    if (typeof mask === 'string') {
        return mask;
    }
    return mask[index] || ' ';
}
function defaultIsCharacterSame({ currentValue, formattedValue, currentValueIndex, formattedValueIndex, }) {
    return currentValue[currentValueIndex] === formattedValue[formattedValueIndex];
}
export function getCaretPosition(newFormattedValue, lastFormattedValue, curValue, curCaretPos, boundary, isValidInputCharacter, 
/**
...
export function getCaretPosInBoundary(value, caretPos, boundary, direction) {
...
export function caretUnknownFormatBoundary(formattedValue) {
...
export function useInternalValues(value, defaultValue, valueIsNumericString, format, removeFormatting, onValueChange = noop) {
    const getValues = usePersistentCallback((value, valueIsNumericString) => {
    ...
    const _onValueChange = (newValues, sourceInfo) => {
